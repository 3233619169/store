...
if ($islogin2 == 1) {
    ...
    if ($userrow['power'] == 2) {
        $level = '<font color="orange">合伙人售价</font>';
    } elseif ($userrow['power'] == 1) {
        $level = '<font color="red">加盟商售价</font>';
    }
...
$share_link = '我钱不够买这个东西，能够帮我买一下嘛~，这是付款订单,谢谢啦 ' . $siteurl . '?store=order&orderid=' . $orderid . '&set';
...
<div class="layui-container" style="padding: 0">
    <div class="layui-row layui-col-space8">
        <div class="layui-col-xs12"
            ...
            <a href="javascript:history.go(-1)">
                <p style="margin-top: 0.5em;font-size: 1.2em"><span class="layui-icon layui-icon-left"></span> 确认订单
                </p>
            </a>
        </div>
        ...
        <div class="layui-col-xs12" style="padding: 0.1em 1em 0 1em;">
            <div class="layui-card" style="border-radius: 0.5em">
                <div class="layui-card-header">
                    <i class="layui-icon layui-icon-component"></i> <?php echo $conf['sitename'] ?>
                ...
                <div class="layui-row layui-col-space8">
                    <div class="layui-col-xs2">
                        <div style="border-radius: 0.5em;text-align: center;margin: 0.3em;box-shadow: 3px 3px 16px #eee"
                             id="layer-photos-demo" class="layer-photos-demo">
                            <img alt="<?php echo $tool['name'] ?>"
                                 layer-src="<?php echo $tool['shopimg'] ? $tool['shopimg'] : 'assets/storepdd/picture/error_img.png' ?>"
                                 src="<?php echo $tool['shopimg'] ? $tool['shopimg'] : 'assets/storepdd/picture/error_img.png' ?>"
                                 style="width: 100%;border-radius: 0.5em"/>
                        </div>
                    </div>
                   <div class="layui-card-header"
                             style="font-size: 0.8rem;line-height: 3.4em;height:auto;">
                            <?php echo $tool['name'] ?>
                        </div>
                        <br>
                        <div class="layui-card-body information layui-text" style="line-height: 2em">
                            <p>购买数量<span style="float: right;"><?php echo $row['num'] ?>个</span></p>
                            <p>售价等级<span style="float: right;"><?php echo $level ?></span></p>
                            <p>商品售价<span style="float: right;"><?php echo $row['money'] ?> 元</span>
                            </p>
                            <p style="float: right;margin-top: 0.8em;font-size: 1.05em">共<?php echo $row['num'] ?>份
                                小计：<font color="#ff4500" style="font-size: 1.1em">￥<?php echo $row['money'] ?></font>
                            </p>
                        </div>
                    </div>
                </div>
 ...
 <?php if ($conf['wxpay_api'] != 0) { ?>
                        <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                            <input type="radio" name="pay" <?php echo($conf['wxpay_api'] != 0 ? '' : 'disabled') ?>
                                   value="wxpay"
                                   title="<p style='width:100%'><img src='./assets/img/wxpay.png' width='13px' /> 微信（官方收取3%手续费）</p>">
                        </div>
                    <?php } ?>
                    ...
                    <?php if ($conf['qqpay_api'] != 0) { ?>
                        <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                            <input type="radio" name="pay" <?php echo($conf['qqpay_api'] != 0 ? '' : 'disabled') ?>
                                   value="qqpay"
                                   title="<p style='width:100%'><img src='./assets/img/qqpay.png' width='13px' /> QQ（官方收取3%手续费）</p>">
                        </div>
                    <?php } ?>
                        <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                           <a href="./?store=wx"> <input type="radio" name="pay" 
                                   value="qqpay"
                                   title="<p style='width:100%'><img src='./assets/img/wxpay.png' width='13px' /> QQ钱包转微信余额</p>"></a>
                        </div>
                    <div id="demo_url" data-url="<?php echo $share_link ?>"
                         style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em">
                     ...
                     <div class="layui-col-xs12"
             style="z-index: 3;;background-color: white;bottom: 0;left: 0;height: 5em;line-height: 5em;box-shadow: 0px 0px 8px #ccc;text-align: right;border-radius:9px">
            <input type="hidden" id="orderid" value="<?php echo $orderid ?>">
            <input type="hidden" id="tid" value="<?php echo $row['tid'] ?>">
            <span style="font-size: 1.2em;float: left;text-indent: 0.5em"><span style="color: #A6A6A6;font-size: 0.9em">共<?php echo $row['num'] ?>份，</span><span>合计：</span><font
                        color="#ff4500">￥<?php echo $row['money'] ?>元</font></span>
            <button class="layui-btn layui-btn-danger layui-btn-radius"
                    style="background: linear-gradient(to right, #f85032, #e73827);margin-right: 0.5em;height:3.5rem;width:8rem;font-size:1rem" id="dopay">立即支付
            </button><hr>
        </div> 
            </div>
...
<script src="assets/stores/js/order.js?ver=<?php echo VERSION ?>"></script>
<script>
    layer.photos({
